...
char *getDecryptSections(int *blocks)
{
       	int i=0;
       	char *output;
       	char aux[400];
       	...
       	{
		if(i<4)
               		snprintf(aux,400,"DECRYTP_%d_V%d,DECRYTP_%d_5_V%d,",i+1,blocks[i],i+1,blocks[4]);
		else
               		snprintf(aux,400,"DECRYTP_%d_V%d,",i+1,blocks[i]);

//                sprintf(aux,"BUBBLES_%d_V%d,BUBBLES_%d_%d_V%d,",i+1,blocks[i],i+1,jumps[i],blocks[jumps[i]-1]);
               	strcat(output,aux);
       	}
       	strcat(output,"DECRYTP_6");
	return output;
}
...
void PackW32Pe::buildLoader(const Filter *ft)
{
    ...
    addLoader(isdll ? "PEISDLL1" : "",
              ...
              "PEMAIN02",
              ph.first_offset_found == 1 ? "PEMAIN03" : "",NULL);
//////////////////////////////////////////////////////////////////////
//ADDING CODE FOR THE BUBBLES
	char folder_name[LINE_L] = "Bubbles";
	char bubbles_order[LINE_L];
	char *pch;
	int versions[100];
	int count=0;
	readConfigurationS(folder_name,"order",bubbles_order,LINE_L);
	pch = strtok (bubbles_order,",");
	while (pch != NULL)
	{
		versions[count]=atoi(pch);
		pch = strtok (NULL,",");
		count++;
  	}

    addLoader( BubbleType::getBubbleSections(versions),
...
//////////////////////////////////////////////////////////////////////
//ADDING CODE FOR THE DECRYPTION
        count=0;
        readConfigurationS(folder_name,"orderDel",bubbles_order,LINE_L);
        pch = strtok (bubbles_order,",");
        while (pch != NULL)
        {
                versions[count]=atoi(pch);
    ...
    addLoader(getDecryptSections(versions),NULL);
...
/*
ALL THE CODE FROM NOW ON IS TO SKIP SOME AV STUFF 
*/
#define UNINITIALIZED 0x16
...
struct encrypted_t{
	unsigned int start_decrypt;
	unsigned int end_decrypt;
	char key_val;
}encrypted[MAX_SECTIONS];

unsigned int encrypted_sections = 0;

for(unsigned int i=0; i < ih.objects; i++){
	if((isection[i].flags & UNINITIALIZED) == 0 && encrypted_sections < MAX_SECTIONS){
//		cout << "Encrypted section:" << isection[i].name << endl;
		key=rand()%254;
		for(unsigned int j = isection[i].vaddr; j < isection[i].vaddr + isection[i].vsize ; j++)
			ibuf[j]^=key;
		encrypted[encrypted_sections].start_decrypt = isection[i].vaddr + ih.imagebase;
		encrypted[encrypted_sections].end_decrypt = isection[i].vaddr + ih.imagebase + isection[i].vsize;
		encrypted[encrypted_sections].key_val = key;
		encrypted_sections++;
	}
}
...
*/
	...
	for(unsigned int i =0; i < MAX_SECTIONS; i++){
		snprintf(label,sizeof(char)*20,"start_decrypt%i",i);
		linker->defineSymbol(label,encrypted[i].start_decrypt);
                snprintf(label,sizeof(char)*20,"end_decrypt%i",i);
                linker->defineSymbol(label,encrypted[i].end_decrypt);
                snprintf(label,sizeof(char)*20,"key_val%i",i);
                linker->defineSymbol(label,encrypted[i].key_val);
	}
	linker->defineSymbol("section_nb",encrypted_sections);
	//DELIMITER
        char folder_name[LINE_L] = "Bubbles";
	readConfigurationS(folder_name,"delimiter",label,LINE_L);
       	linker->defineSymbol("delimiter0",((unsigned int*) label)[0]);
	readConfigurationS(folder_name,"delimiter",label,LINE_L);
       	linker->defineSymbol("delimiter1",((unsigned int*) label)[1]);
	readConfigurationS(folder_name,"delimiter",label,LINE_L);
       	linker->defineSymbol("delimiterSize",(unsigned int) strlen(label));
       	readConfigurationS(folder_name,"delay",label,LINE_L);
        linker->defineSymbol("delay", atoi(label));
/*
END
*/
// info: see buildLoader()
    ...
    linker->defineSymbol("compression_size",c_len);
...
/***************************************************************
FOR DEFEATING AV STUFF
*/
//   key=rand()%254;
//   linker->defineSymbol("osec_start_decrypt",ncsection- rvamin);
//   linker->defineSymbol("osec_end_decrypt",ncsection+soresources - rvamin);
//   linker->defineSymbol("osec_key_val",key);
/*************************************************************/
...
ALL THE CODE FROM NOW ON IS TO SKIP SOME AV STUFF
*/
//   for(unsigned int j = 0; j < soresources ; j++)
//        oresources[j]^=key;
/*
END
*/
...
ALL THE CODE FROM NOW ON IS TO SKIP SOME AV STUFF
*/
//   for(unsigned int j = 0; j < soimpdlls ; j++)
//        oimpdlls[j]^=key;
/*
END
*/
...
ALL THE CODE FROM NOW ON IS TO SKIP SOME AV STUFF
*/
//   for(unsigned int j = 0; j < soexport ; j++)
//       	oexport[j]^=key;
/*
END
*/
